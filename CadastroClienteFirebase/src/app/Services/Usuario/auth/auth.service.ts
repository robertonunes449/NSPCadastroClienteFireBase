 ...
 criarUser(user: any): Promise<any> {
  ...
  resetPassword(email: string): Promise<any> {
      return this.afAuth.auth.sendPasswordResetEmail(email)
          .then(() => {
              console.log('Auth Service: redefinição de senha feita com sucesso');
              // this.router.navigate(['']);
          })
          .catch(error => {
              console.log('Auth Service:erro de redefinição de senha ...');
  ...
  async resendVerificationEmail() {                         // verification email is sent in the Sign Up function, but if you need to resend, call this function
      return (await this.afAuth.auth.currentUser).sendEmailVerification()
          ...
          .catch(error => {
              console.log('Auth Service:erro ao enviar Email de verificação...');
              console.log('error code', error.code);
              console.log('error', error);
              if (error.code)
                  return error;
  ...
  setUserInfo(payload: object) {
      console.log('Auth Service: salvando informação de usuário...');
      this.afs.collection('usuario')
          .add(payload).then(function (res) {
              console.log("Auth Service:resposta em obter informações do usuário ...")
              console.log(res);
          })
  }
